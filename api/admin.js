...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "block_ip") {
      ...
      global.activeUsers = global.activeUsers.filter(user => user.ip !== ip);
      return sendJson(res, 200, { status: true, message: `IP ${ip} berhasil diblokir.` });
    }
  }
